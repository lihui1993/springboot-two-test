...
import cn.allcheer.springbootbylihui.utils.baofu.rsa.RsaCodingUtil;
...
//		 DateArry.put("risk_item",JSONObject.fromObject(RiskItem).toString());//放入风控参数
		 ...
		 if(ReturnData.get("resp_code").toString().equals("S")){
			...
			log.info("预支付唯一码:"+SecurityUtil.Base64Decode(SecurityUtil.AesDecrypt(ReturnData.get("unique_code"),RAesKey)));
		 }else{
			 log.info("失败！");
		 }
	}
